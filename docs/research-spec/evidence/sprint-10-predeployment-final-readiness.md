...
- **Code SHA Tested**: abd91eaa33579fd840ace4c258b8828b4d5ac614
- **Pre-Deployment Final Readiness Gate**: **PASS**
- **Final ASUS Deployment Gate**: **DEFERRED**
- **Timestamp (UTC)**: 2026-08-29T06:41:00Z
...
- **Workflow Run ID**: `33238979739`
- **Commit Tested**: `abd91eaa33579fd840ace4c258b8828b4d5ac614`
...
- **Job 1 (Unit & Integration Tests - Python 3.11)**:
  - Duration: 56s
  - Result: `SUCCESS`
  - Total Tests: 250 passed (0 failed, 0 errors, 0 skipped on Linux CI)
...
| **State UID/GID 10001 Validation** | `scripts/deploy/validate_state_dir.py` validates POSIX `10001:10001` | `test_state_dir_validation_behavioral_wrong_uid_gid`, `test_state_dir_validation_behavioral_mock_uid_gid` | `PASS` |
| **State Execute/Traverse Validation** | `scripts/deploy/validate_state_dir.py` checks `S_IXUSR` (rejects 0640, requires 0750) | `test_state_dir_validation_behavioral_rejects_missing_execute_0640` | `PASS` |
...
| **Behavioral Empty API Key Preflight** | `asus-preflight.sh` fails with non-zero exit when `RBTA_API_KEY=` in `.env` | `test_preflight_behavioral_empty_api_key` | `PASS` |
| **Behavioral Empty Model Version Preflight** | `asus-preflight.sh` fails with non-zero exit when `RBTA_MODEL_VERSION=` in `.env` | `test_preflight_behavioral_empty_model_version` | `PASS` |
| **Behavioral Compose API Key Fail-Closed** | `docker compose config` fails with non-zero exit when `RBTA_API_KEY` is missing | `test_compose_behavioral_fail_closed_missing_api_key` | `PASS` |
| **Behavioral Compose Model Fail-Closed** | `docker compose config` fails with non-zero exit when `RBTA_MODEL_VERSION` is missing | `test_compose_behavioral_fail_closed_missing_model_version` | `PASS` |
...
| **Behavioral Smoke Auth Fail-Closed** | `smoke.sh` exits non-zero before network call if `RBTA_API_KEY` is unset | `test_smoke_script_behavioral_auth_fail_closed` | `PASS` |
| **Behavioral Missing Env Check** | `asus-preflight.sh` exits non-zero when `RBTA_ENV_FILE` does not exist | `test_preflight_behavioral_missing_env` | `PASS` |
...
1. **PRE-DEPLOYMENT FINAL READINESS GATE**: **PASS**  
   *Canonical single-layout directory structure, UID/GID 10001 state ownership + execute/traverse validator, behavioral fail-closed tests, Dockerfile provenance, and CI verification workflows are 100% complete and passing.*
